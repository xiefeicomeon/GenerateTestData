...
def run_state(state):
    if state == "generate":
        read_file(cn.feeder_file_path)
        generate_test_data(cn.feeder_file_path)
        calculate_theoretical_data(cn.feeder_file_path)
    if state == "analyse":
        analyse_data(cn.feeder_file_path)
        generate_result(cn.feeder_file_path)
